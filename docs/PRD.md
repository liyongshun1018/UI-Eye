...
1. **AI 诊断深度优化** ✅
   - 引入“二次深度扫描”策略，由 AI 对大面积差异区进行组件级拆解。
   - 结构化日志记录，完整展示 AI 的 Reasoning 过程与 Prompt 细节。
   - 整合视觉设计原则（对比度、对齐、层次）至诊断算法。

2. **视口智能自适应与对齐修正** ✅
   - 自动探测设计稿物理宽度，告别手动视口配置。
   - 实现截图后的“物理裁剪”保障机制，解决由于页面元素溢出导致的超宽截图对齐失效问题。

3. **冗余功能精简** ✅
   - 移除了低频且不稳定的 PDF 导出功能，回归纯粹的 Web 交互式报告。
   - 统一全局视图宽度，优化暗黑模式下的视觉呈现。
...
1. **架构重构 (TypeScript + DDD)** ✅
   - 后端代码全量迁移至 TypeScript。
   - 采用领域驱动设计 (DDD) 架构，分层清晰（Domain, Application, Infrastructure, Interfaces）。
   - 引入依赖注入 (IoC) 容器，提升代码解耦度。

2. **批量审计流水线 (Batch Audit)** ✅
   - 支持多 URL 批量截图、对比及报告生成。
   - 集成 WebSocket 实现任务进度的实时精准推送。
   - 提供批量任务管理界面及其结果汇总仪表盘。

3. **Chrome 插件集成 (Extension)** ✅
   - 开发配套浏览器插件，支持实时点选元素进行 AI 视觉诊断。
   - 完美解决单页应用 (SPA) 登录态及复杂交互场景下的审计难题。

4. **自动化脚本机制 (Scripts)** ✅
   - 支持编写自定义 Playwright/Puppeteer 脚本，在审计前处理登录、弹窗、动态交互等逻辑。
   - 集成 CodeMirror 专业编辑器，支持语法高亮与代码建议。

5. **数据持久化增强 (SQLite)** ✅
   - 统一使用关系型数据库管理任务、报告与脚本，支持更复杂的查询与统计。
...
1. **多引擎对比** ✅
   - 集成 ODiff 高性能引擎，对比速度提升 3-5 倍
   - 支持 Pixelmatch、Resemble.js、ODiff 三种引擎切换
   - 用户可根据场景选择最适合的对比算法
...
| 功能点         | 优先级 | 实现状态      | 详细说明                                     |
| -------------- | ------ | ------------- | -------------------------------------------- |
| 图像对齐       | P0     | ✅ 已实现      | 自动对齐两张图片（处理尺寸差异）             |
| **多引擎支持** | P0     | ✅ **Phase 2** | 支持 Pixelmatch/Resemble/ODiff 三种引擎      |
| **智能吸附**   | P0     | ✅ **Phase 2** | ±2px 自动对齐，减少渲染抖动误报              |
| 像素对比       | P0     | ✅ 已实现      | 逐像素对比，检测颜色、位置差异               |
| 差异高亮       | P0     | ✅ 已实现      | 生成差异图，用红色标注不同区域               |
| **差异聚类**   | P0     | ✅ **Phase 2** | 智能聚类差异区域，生成语义化区块             |
| 相似度评分     | P0     | ✅ 已实现      | 计算整体相似度（0-100%）                     |
| 容差配置       | P1     | ✅ 已实现      | 支持设置颜色容差（如：RGB ±5 视为相同）      |
| 区域忽略       | P2     | ⏳ 计划中      | 支持手动标注忽略区域（如：动态内容、广告位） |
| 分区对比       | P2     | ⏳ 计划中      | 按模块分区对比（头部、内容区、底部等）       |

**算法选型**：
- ✅ **Pixelmatch**：轻量级像素对比库，适合 Web 环境
- ✅ **Resemble.js**：支持抗锯齿检测，适合字体渲染场景
- ✅ **ODiff** (Phase 2 新增)：Rust 实现的高性能引擎，速度提升 3-5 倍

**Phase 2 增强功能**：
```javascript
// 智能吸附算法示例
const bestAlignment = await findBestAlignment(img1, img2, {
  searchRange: 2,  // ±2px 搜索范围
  threshold: 0.1   // 容差阈值
});

// 差异聚类
const diffRegions = await analyzeDiffRegions(diffImagePath);
// 输出: [{ id: 1, x: 100, y: 200, width: 50, height: 30, area: 1500 }]
```
...
| 功能点         | 优先级 | 实现状态      | 详细说明                                   |
| -------------- | ------ | ------------- | ------------------------------------------ |
| 对比视图       | P0     | ✅ 已实现      | 左右并排展示设计稿与实际页面               |
| 滑动对比       | P0     | ✅ 已实现      | 拖动滑块切换两张图片（类似 Before/After）  |
| 差异叠加       | P1     | ✅ 已实现      | 将差异图叠加在原图上，可调节透明度         |
| **SVG 交互层** | P0     | ✅ **Phase 2** | 动态 SVG 矩形高亮差异区域                  |
| **双轴联动**   | P0     | ✅ **Phase 2** | 点击图表定位列表，点击列表高亮图表         |
| **CSS 预览**   | P0     | ✅ **Phase 2** | 实时预览 CSS 修复效果                      |
| 还原度仪表盘   | P0     | ✅ 已实现      | 环形进度条展示还原度分数                   |
| 修复建议列表   | P0     | ✅ 已实现      | 分类展示所有修复建议，支持一键复制代码     |
| 缩放与标注     | P1     | ✅ 已实现      | 支持放大查看细节，点击差异区域查看对应建议 |
| 导出报告       | P1     | ⏳ 计划中      | 导出为 PDF 或 HTML 格式，便于分享          |
| 历史对比       | P2     | ✅ 已实现      | 查看历史对比记录，追踪还原度变化趋势       |
...
```vue
<!-- 动态渲染差异区域 -->
<svg class="diff-overlay">
  <rect v-for="region in diffRegions"
    :x="region.x" :y="region.y"
    :width="region.width" :height="region.height"
    @click="highlightRegion(region.id)" />
</svg>
```
...
```javascript
// 通过后端代理预览修复效果
const previewUrl = `/api/proxy-preview?url=${pageUrl}&css=${encodeURIComponent(fixCSS)}`;
```
...
| 性能       | 快                           | 稍慢                    |
| 生态成熟度 | 高                           | 中                      |
| **选择**   | ✅ 选用 (已升级至 Playwright) | -                       |

**决策**：项目已从 Puppeteer 升级至 **Playwright**，以获得更好的跨平台稳定性、更强的多设备模拟能力以及对复杂交互脚本的深度支持。
