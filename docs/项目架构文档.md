...
graph TD
    %% ========== 第一层：表现层 - 用户交互界面 ==========
    Web["🌐 Vue 3 Web 应用<br/>批量任务 | 单次对比 | 脚本管理"]
    ...
    %% ========== 第三层：应用层 - 业务用例编排 ==========
    UC1["📋 批量任务管理<br/>多 URL 并发处理"]
    UC2["🔄 单次对比流程<br/>截图 → 比对 → AI 诊断"]
    UC3["📝 脚本管理<br/>交互脚本 CRUD"]
    
    %% ========== 第四层：领域层 - 核心业务模型 ==========
    Model["📦 领域模型<br/>BatchTask | Script | Report"]
    Repo["🗂️ 仓储接口<br/>数据访问抽象"]
    
    %% ========== 第五层：基础设施层 - 技术实现 ==========
    Capture["📸 Playwright 截图引擎"]
    Compare["🔍 ODiff 像素比对"]
    AI["🤖 SiliconFlow AI 诊断"]
    DB["💾 SQLite 数据持久化"]
    Storage["📁 文件存储"]
    
    %% ========== 第六层：外部依赖 - 第三方服务 ==========
    Browser["🌐 Playwright 浏览器"]
    AIService["☁️ SiliconFlow API"]
    FileSystem["💿 文件系统"]
    Database["🗄️ SQLite 数据库"]
...
sequenceDiagram
    ...
    loop 遍历每个 URL
        ...
        Note over 对比流程: 步骤1: 视口宽度智能探测
        对比流程->>对比流程: 自动读取关联设计稿宽度 (Viewport Auto Detect)
        
        Note over 对比流程: 步骤2: 执行截图
        对比流程->>Playwright: 启动浏览器并截图
        Playwright->>Playwright: 设置视口和移动端模拟
        Playwright->>Playwright: 执行交互脚本(如登录)
        Playwright->>Playwright: 属性截图 (FullPage)
        Playwright-->>对比流程: 返回截图文件
        
        Note over 对比流程: 步骤3: 物理裁剪保障
        对比流程->>对比流程: 强制将截图裁剪至探测宽度 (解决溢出问题)
        ...
        alt 有设计稿
            ...
            Note over 对比流程: 步骤5: AI 深度审计
            对比流程->>AI诊断: 二次扫描差异区块
            AI诊断->>AI诊断: 结构化 Prompt 构建
            AI诊断-->>对比流程: 详细诊断日志与 CSS 修复建议
        end
...
sequenceDiagram
    ...
    alt 用户选择应用修复
        ...
        Content Script->>页面DOM: 动态注入样式 (Live Fix)
        页面DOM-->>用户: 实时预览效果
    end
```
